...
public class QuickSort extends JFrame{
    ...
    private static void quick(LocalDate []vetor, int esquerda, int direita){
        if(esquerda < direita){
            int x = separar(vetor, esquerda, direita);
            quick(vetor, esquerda, x-1);
            quick(vetor, x+1, direita);
        };
    };
    
    private static int separar(LocalDate []vetor, int esquerda, int direita){
        int i = esquerda + 1;
        int x = direita;
        LocalDate pivo = vetor[esquerda];
        while(i <= x){
            if((vetor[i].compareTo(pivo)) <= 0){
                i++;
            } else if((vetor[x].compareTo(pivo)) > 0){
                x--;
            } else if(i <= x){
                trocarPosicao(vetor, i, x);
                i++;
                x--;
            };
        };
        trocarPosicao(vetor, esquerda, x);
        return x;
    };
    
    private static void trocarPosicao(LocalDate []vetor, int i, int x){
    ...
    public void run(ActionListener botao1, String query1, String query2, String query3, String query4, String tipoOrdem) {
        ...
        JLabel titulo = new JLabel("Método Quick Sort – Ordem " + tipoOrdem);
